port module FourSeasonsApp exposing (..)

import Html exposing (Html, Attribute, text, audio, div, pre, ul, li)
import Html.Attributes exposing (src, controls, style, classList, type_, class)
...
percentLength = 8.3333
progressToTime: Bool -> Float -> Float
progressToTime equalizeSizes progress =
  case equalizeSizes of
    True -> distortedProgressToTime progress
    False -> progress * fullLength / 100
  
distortedProgressToTime: Float -> Float
distortedProgressToTime progress =
  let
    movementNumber = floor (progress / percentLength)
    movement = case (movements |> List.drop movementNumber |> List.head) of
      Nothing -> defaultMovement
      Just m -> m
    relativeProgress = (progress - (toFloat movementNumber * percentLength)) * 12.0
    relativeTime = movement.length * (relativeProgress / 100)
    adjustedTime = movement.start + relativeTime
...
timeToProgress equalizeSizes time =
  case equalizeSizes of
    True -> timeToDistortedProgress time
    False -> time * 100 / fullLength

timeToDistortedProgress: Float -> Float
timeToDistortedProgress time =
  let
    currentMovement = Movement.currentFromTime time
    baseProgress = toFloat currentMovement.index * percentLength
    extraProgress = (time - currentMovement.start) * percentLength / currentMovement.length
    _ = Debug.log "debug 77:" (baseProgress, extraProgress)
  in
...
debugInfo model =
  let
    currentMovement = Movement.currentFromTime model.currentTime
  in
    pre [] [
      text (toString (Utils.timeToDate model.currentTime) ++ "\n"
...
view model =
  ...
  in
    div [class "container",style [("background", Utils.color currentMovement 50 70)]] [
      audio [Html.Attributes.id "player"
    ...
    , div [class "seeker"] [
      ...
      , Html.input [  type_ "range", Html.Attributes.max "100", Html.Attributes.step "0.01"
...
movementToLi  m model =
  let
    currentMovement = Movement.currentFromTime model.currentTime
    width = case model.equalizeSizes of
      True -> percentLength
      False -> m.length / fullLength * 100
  in
    li [ classList [("active", m == currentMovement)]
