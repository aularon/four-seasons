module Music.Movement exposing (..)
...
next currentMovement =
  let
    loopingMovements = movements

    nextFromMovements l = case l of
      x :: [] -> firstMovement
      x :: y :: rest -> if x == currentMovement
                        then y
                        else nextFromMovements (y :: rest)
      _ -> defaultMovement
  in
    nextFromMovements loopingMovements

ends: Movement -> Time
ends m = m.start + m.length
...
remaininigTillNext currentTime = 
  ...
  let
    current = currentFromTime currentTime
  in
